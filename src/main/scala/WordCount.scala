...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    /**
      * RDD操作无法直接实现，用文件操作进行修改，然后再进行RDD操作
      */
...
*/
    ...
    //写成每行逗号后面跟记录号的形式，后面再用split（“，”）分隔
    /*
        val reader = Source.fromFile("E:\\ECG\\心电文件\\CCDD\\1by1.txt")
        val writer = new PrintWriter(new File("E:\\ECG\\心电文件\\CCDD\\Test1.txt"))
        for(line <- reader.getLines()) {

          if (line.length > 3){
            val num = line.substring(0,4)
            writer.write(num.concat("\r\n"))
          }
          else{
            println(line)
          }
...
//    line.flatMap(_.split("\\t")).foreach(println)
 ...
 /**统计三级病症的记录条数（读取文件为ccdd_labels_HexCode.txt）
    line.flatMap(_.split(",")).map(_.replaceAll("[0-9]+[\\t]" , "")).map((_,1)).reduceByKey(_+_).collect().foreach(println)
*/

/**
  * 按类别统计一级病症的记录（读取文件为sparkTest2.txt）
  */
    line.map(_.replaceAll("[0-9a-z]{4}[\n]" , "\n")).saveAsTextFile("C:\\data")


//      .saveAsTextFile("E:\\ECG\\心电文件\\CCDD\\1by1.txt")
...
//    val record = line.flatMap(_.split(","))
//
//    val num = record.map(x => x.split("\\t")(0))
//
//    val diseaseNum = record.map(x => x.split("\\t")(1))

//    val recordNew = new Tuple2[String, String](num, diseaseNum)

//    val recordNew = List(num, diseaseNum)
//
//    println(recordNew)
